...
void boot_default_servers(void)
{
        ...
        srv_path = "/lwip.srv";
        proc_node = procmgr_launch_process(
                1, &srv_path, "lwip", true, INIT_BADGE, SYSTEM_SERVER);
        if (proc_node == NULL) {
...
#define CONFIG_MACHINE_ARM 0
        ...
        procmgr_launch_process(
                1, args, "machine_arm", true, INIT_BADGE, COMMON_APP);
#endif

        /* Start shell. */
        procmgr_launch_process(
                1, &shell_argv, "chcore-shell", true, INIT_BADGE, COMMON_APP);
#if defined(CHCORE_PLAT_RASPI3) && defined(CHCORE_SERVER_GUI)
        char *terminal_argv = "terminal.bin";
        procmgr_launch_process(
                1, &terminal_argv, "terminal", true, INIT_BADGE, COMMON_APP);
#endif
}
