...
int chcore_poll(struct pollfd fds[], nfds_t nfds, int timeout)
{
        ...
        poll_notifc = usys_create_notifc();
        if (poll_notifc < 0)
                return poll_notifc;
...
#ifdef CHCORE_SPLIT_CONTAINER
                usys_wait(poll_notifc, true, NULL);
#else /* CHCORE_SPLILT_CONTAINER */
                usys_yield();
...
#ifdef CHCORE_SPLIT_CONTAINER
        usys_revoke_cap(poll_notifc, false);
#endif /* CHCORES_PLIT_CONTAINER */
