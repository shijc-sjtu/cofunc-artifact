...
static void eventfd_notify_reader(struct eventfd *efdp)
{
        usys_notify(efdp->reader_notifc_cap);
}

static void eventfd_notify_writer(struct eventfd *efdp)
{
        usys_notify(efdp->writer_notifc_cap);
}
...
int chcore_eventfd(unsigned int count, int flags)
{
        ...
        if ((notifc_cap = usys_create_notifc()) < 0) {
                ret = notifc_cap;
                goto fail;
        }
...
static ssize_t chcore_eventfd_read(int fd, void *buf, size_t count)
{
        ...
        while (1) {
                ...
                ret = usys_wait(efdp->reader_notifc_cap, true, 0);
                BUG_ON(ret);
        }

out:
#ifdef CHCORE_SPLIT_CONTAINER
        if (efdp->pollarg.poll_notifc >= 0 &&
            (efdp->pollarg.events & (POLLOUT | POLLWRNORM))) {
                usys_notify(efdp->pollarg.poll_notifc);
                efdp->pollarg.poll_notifc = -1;
        }
#endif /* CHCORE_SPLIT_CONTAINER */
        chcore_spin_unlock(&efdp->efd_lock);
...
static ssize_t chcore_eventfd_write(int fd, void *buf, size_t count)
{
        ...
        while (1) {
                ...
                ret = usys_wait(efdp->writer_notifc_cap, true, 0);
                BUG_ON(ret);
        }
...
#ifdef CHCORE_SPLIT_CONTAINER
        if (efdp->pollarg.poll_notifc >= 0 &&
            (efdp->pollarg.events & (POLLIN | POLLRDNORM))) {
                usys_notify(efdp->pollarg.poll_notifc);
                efdp->pollarg.poll_notifc = -1;
        }
#endif /* CHCORE_SPLIT_CONTAINER */
