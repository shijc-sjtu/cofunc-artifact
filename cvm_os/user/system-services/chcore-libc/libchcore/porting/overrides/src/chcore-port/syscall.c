...
long usys_ipc_call(cap_t conn_cap, unsigned long ipc_msg_ptr, unsigned int cap_num)
{
        return chcore_syscall3(
                CHCORE_SYS_ipc_call, conn_cap, ipc_msg_ptr, cap_num);
}
...
{
#if defined(CHCORE_SPLIT_CONTAINER) && !defined(CHCORE_SPLIT_CONTAINER_SYNC)
        return sc_create_notifc();
#endif
...
{
#if defined(CHCORE_SPLIT_CONTAINER) && !defined(CHCORE_SPLIT_CONTAINER_SYNC)
        return sc_wait(notifc_cap, is_block, timeout);
#endif
        return chcore_syscall3(
...
#ifdef CHCORE_SPLIT_CONTAINER
long usys_split_container(int op, long a, long b, long c, long d)
{
        return chcore_syscall5(CHCORE_SYS_split_container, op, a, b, c, d);
}

cap_t usys_clone_pmo(cap_t pmo_cap)
{
        return chcore_syscall1(CHCORE_SYS_clone_pmo, pmo_cap);
}
#endif
