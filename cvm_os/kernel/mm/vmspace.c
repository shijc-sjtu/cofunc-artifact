...
static int fill_page_table(struct vmspace *vmspace, struct vmregion *vmr)
{
        ...
        pa = vmr->pmo->start + vmr->offset;
        va = vmr->start;

        lock(&vmspace->pgtbl_lock);
        ret = map_range_in_pgtbl(
                vmspace->pgtbl, va, pa, vmr->size, vmr->perm, &rss);
        vmspace->rss += rss;
...
{
        int ret, pmo_type;
        struct vmregion *dst_vmr;
        struct vmregion *src_vmr;
        struct pmobject *pmo;
        
        for_each_in_list (src_vmr, struct vmregion, list_node,
                          &src_vmspace->vmr_list) {
                pmo = src_vmr->pmo;
                pmo_type = pmo->type;
                BUG_ON(src_vmr->perm & VMR_COW);

                if (pmo_type == PMO_SC_SHM) {
                        pmo = obj_get(dst_cap_group, (cap_t)(long)pmo->private, TYPE_PMO);
                        obj_put(pmo);
                        BUG_ON(pmo->type != PMO_SC_SHM);
                }

                dst_vmr = alloc_vmregion(src_vmr->start, src_vmr->size, src_vmr->offset,
                                         src_vmr->perm, pmo);
                if (pmo_type == PMO_DATA || pmo_type == PMO_ANONYM) {
                        dst_vmr->perm &= ~VMR_WRITE;
                        dst_vmr->perm |= VMR_COW;
                }
                
                ret = add_vmr_to_vmspace(dst_vmspace, dst_vmr);
                BUG_ON(ret < 0);

                if (pmo_type == PMO_DATA || pmo_type == PMO_SC_SHM) {
			fill_page_table(dst_vmspace, dst_vmr);
                } else {
                        BUG_ON(pmo_type != PMO_ANONYM && pmo_type != PMO_FORBID);
                }

                if (src_vmr == src_vmspace->heap_boundary_vmr) {
			dst_vmspace->heap_boundary_vmr = dst_vmr;
                }
        }
                      ...
                      vmr_prop_t flags, struct pmobject *pmo)
{
        ...
        if ((pmo->type == PMO_DATA) || (pmo->type == PMO_DATA_NOCACHE)
            || (pmo->type == PMO_DEVICE) || (pmo->type == PMO_SC_SHM))
