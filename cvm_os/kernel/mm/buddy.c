                                    ...
                                    struct page *chunk)
{
        ...
        return __virt_to_page(pool, (void *)buddy_chunk_addr);
}
...
struct page *__buddy_get_pages(struct phys_mem_pool *pool, int order)
{
        int cur_order;
        struct list_head *free_list;
...
struct page *buddy_get_pages(struct phys_mem_pool *pool, int order)
{
        ...
        page = __buddy_get_pages(pool, order);

        unlock(&pool->buddy_lock);
        return page;
}

void __buddy_free_pages(struct phys_mem_pool *pool, struct page *page)
{
        int order;
        struct list_head *free_list;
...
struct page *__virt_to_page(struct phys_mem_pool *pool, void *ptr)
{
        struct page *page;
...
struct page *virt_to_page(void *ptr)
{
        ...
        return __virt_to_page(pool, ptr);
}
