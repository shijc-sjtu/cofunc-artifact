...
int handle_trans_fault(struct vmspace *vmspace, vaddr_t fault_addr)
{
        ...
        case PMO_SHM: {
                ...
                if (pa == 0) {
                        ...
                        if (perm & VMR_COW) {
                                perm |= VMR_WRITE;
                                vmregion_record_cow_private_page(vmr, fault_addr, new_va);
                        } else {
                        ...
                        if (pmo->type == PMO_SHM || pmo->type == PMO_ANONYM) {
                                ...
                                paddr_t query_pa;
                                pte_t *fault_pte;
                                if (!query_in_pgtbl(vmspace->pgtbl, fault_addr, &query_pa, &fault_pte))
                                        goto unlock;
                                if ((perm & VMR_COW) && !(perm & VMR_WRITE) && (current_cap_group->sc_active_vcpus > 1)) {
                                        void *new_va = get_pages(0);
                                        memcpy(new_va, (void *)phys_to_virt(pa), PAGE_SIZE);
                                        pa = virt_to_phys(new_va);
                                        perm |= VMR_WRITE;
                                        vmregion_record_cow_private_page(vmr, fault_addr, new_va);
                                }
...
out_succ:
        unlock(&vmspace->vmspace_lock);
        flush_tlb_by_range(vmspace, ROUND_DOWN(fault_addr, PAGE_SIZE), PAGE_SIZE);
        return ret;
out_illegal:
