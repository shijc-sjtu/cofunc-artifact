 ...
 * @param shm_start: start address of shared memory of a connection. This address
 * may be server-side or client-side. When checking ipc_msg for client-side, client-
 * side start address should be passed in, and vice versa.
 * @param shm_size: size of shared memory
 * @return int: 0 indicates a legal ipc_msg, otherwise -1 is to indicate invalid or
 * malicious ipc_msg.
 */
// static int check_ipc_msg_in_shm(struct ipc_msg *user_ipc_msg_ptr, vaddr_t shm_start, size_t shm_size)
// {
//         int ret;
                           ...
                           unsigned int cap_num)
{
        ...
        if (ipc_msg_in_client != NULL) {
                ...
                // if (check_ipc_msg_in_shm(ipc_msg_in_client, conn->shm.client_shm_uaddr, conn->shm.shm_size) < 0) {
                //         r = -EINVAL;
