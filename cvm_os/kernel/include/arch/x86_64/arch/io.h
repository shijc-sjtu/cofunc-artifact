...
{
	return sev_snp_do_in(port, IOIO_DATA_8);
}

static inline void put8(u16 port, u8 data)
{
	sev_snp_do_out(port, data, IOIO_DATA_8);
}

static inline void put16(u16 port, u16 data)
{
	sev_snp_do_out(port, data, IOIO_DATA_16);
}

static inline u32 get32(u16 port)
{
	return sev_snp_do_in(port, IOIO_DATA_32);
}

static inline void put32(u16 port, u32 data)
{
	sev_snp_do_out(port, data, IOIO_DATA_32);
}
...
static inline u8 get8(u16 port)
{
	return tdx_do_in(1, port);
}

static inline void put8(u16 port, u8 data)
{
	tdx_do_out(1, port, data);
}

static inline void put16(u16 port, u16 data)
{
	tdx_do_out(2, port, data);
}

static inline u32 get32(u16 port)
{
	return tdx_do_in(4, port);
}

static inline void put32(u16 port, u32 data)
{
	tdx_do_out(4, port, data);
}
#else
