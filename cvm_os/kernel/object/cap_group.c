...
#ifdef CHCORE_SPLIT_CONTAINER
        split_container_cap_group_init(cap_group);
#endif /* CHCORE_SPLIT_CONTAINER */
...
#ifdef CHCORE_SPLIT_CONTAINER
        split_container_cap_group_deinit(cap_group);
#endif /* CHCORE_SPLIT_CONTAINER */
}
...
int alloc_slot_id_at(struct cap_group *cap_group, cap_t slot_id)
{
        ...
        while (slot_id >= slot_table->slots_size) {
                r = expand_slot_table(slot_table);
                if (r < 0) {
                        goto out_fail;
                }
        ...
        if (slot_table->slots_bmp[slot_id / BITS_PER_LONG]
            == ~((unsigned long)0))
