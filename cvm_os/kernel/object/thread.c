...
#ifdef CHCORE_SPLIT_CONTAINER
        split_container_thread_init(thread);
#endif /* CHCORE_SPLIT_CONTAINER */
...
#ifdef CHCORE_SPLIT_CONTAINER
int thread_clone(struct thread *dst_thread, struct thread *src_thread,
                 struct cap_group *dst_cap_group)
{       
        thread_init(dst_thread, dst_cap_group, 0, 0,
	            src_thread->thread_ctx->sc->prio, TYPE_USER,
                    src_thread->thread_ctx->affinity);

	/* Copy the thread's execution context and TLS base */
	memcpy(&dst_thread->thread_ctx->ec,
	       &src_thread->thread_ctx->ec,
	       ARCH_EXEC_CONT_SIZE);
	memcpy(&dst_thread->thread_ctx->tls_base_reg,
	       &src_thread->thread_ctx->tls_base_reg,
	       sizeof(dst_thread->thread_ctx->tls_base_reg));

	list_add(&dst_thread->node, &dst_cap_group->thread_list);
	dst_cap_group->thread_cnt++;
	
        if (src_thread->thread_ctx->state == TS_RUNNING) {
                dst_thread->thread_ctx->state = TS_INTER;
        } else {
                BUG_ON(src_thread->thread_ctx->state != TS_WAITING);
                dst_thread->thread_ctx->state = TS_WAITING;
        }
...
#ifdef CHCORE_SPLIT_CONTAINER
        split_container_thread_deinit(thread);
#endif /* CHCORE_SPLIT_CONTAINER */
                           ...
                           u64 tls)
{
        ...
        if (type == TYPE_USER) {
                ...
                split_container_activate_thread(thread, current_cap_group == cap_group);
