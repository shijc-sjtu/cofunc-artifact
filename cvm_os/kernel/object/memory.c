...
cap_t sys_clone_pmo(cap_t pmo_cap)
{
        ...
        r = create_pmo(pmo->size, PMO_DATA, current_cap_group, 0, &new_pmo);
        if (r < 0) {
                goto out_obj_put;
        }

        memcpy((void*)phys_to_virt(new_pmo->start),
               (void*)phys_to_virt(pmo->start), pmo->size);
                    ...
                    paddr_t paddr)
{
        ...
        case PMO_SC_SHM: {
                BUG_ON(len != (1UL << 12 << 9));
                void *new_va = split_container_get_pages(9, 0);
                if (new_va == NULL)
                        return -ENOMEM;
...
void pmo_deinit(void *pmo_ptr)
{
        ...
        case PMO_SC_SHM: {
                ...
                /* PMO_SC_SHM contains continous physical pages */
                start_addr = pmo->start;
                split_container_free_pages((void *)phys_to_virt(start_addr));
...
int sys_handle_mprotect(unsigned long addr, unsigned long length, int prot)
{
        ...
        while (va < end_va) {
                ...
                if ((vmr->perm & VMR_COW) && (target_prot & VMR_WRITE)) {
                        BUG_ON(addr < vmr->start ||
                               addr + length > vmr->start + vmr->size);
                        target_prot &= ~VMR_WRITE;
                        break;
                }
#endif /* CHCORE_SPLIT_CONTAINER */
        ...
        while (va < end_va) {
                ...
                vmr->perm = target_prot | (vmr->perm & VMR_COW);
