...
static int ptrace_init(int main_thread_cap)
{
	struct ptrace *ptrace;
	int ret = 0, cap;
	struct thread *thread;

	ptrace = obj_alloc(TYPE_PTRACE, sizeof(*ptrace));
	if (!ptrace) {
		ret = -ENOMEM;
		goto out_fail;
	}
	...
	cap = cap_alloc(current_cap_group, ptrace);
	if (cap < 0) {
		ret = cap;
		goto out_free_obj;
	}
	...
	init_notific(&ptrace->singlestep_notifc);
 ...
 * @runnable: suppend all threads (false) or resume all threads' execution (true).
 */
static int ptrace_change_threads_suspend(struct ptrace *ptrace, bool runnable)
{
	...
	for_each_in_list(thread, struct thread, node,
			 &(ptrace->ptrace_proc->thread_list)) {
		thread->thread_ctx->is_suspended = runnable ? SUSPEND : UNSUSPEND;
	}
	unlock(&ptrace->ptrace_proc->threads_lock);
...
static struct thread *find_thread_by_tid(struct ptrace *ptrace, unsigned long tid)
{
	...
	lock(&ptrace->ptrace_proc->threads_lock);
	for_each_in_list(cur_thread, struct thread, node,
			 &(ptrace->ptrace_proc->thread_list)) {
		if (cur_tid == tid) {
			thread = cur_thread;
			break;
...
static int ptrace_getregs(struct ptrace *ptrace, unsigned long tid,
			  void *data)
{
			    ...
			    void *addr, void *data)
{
	...
	copy_ret = copy_from_user_remote(thread->cap_group, (char *)&val, addr, sizeof(long));
	if (copy_ret)
		val = 0;
			    ...
			    void *addr, void *data)
{
	...
	return copy_to_user_remote(thread->cap_group, addr,
				   (char *)&val, sizeof(long));
}

/*
 * Get register (at address @addr, which is architecture-specific) from thread @tid.
 * And store the data to the buffer @data
 */
			    ...
			    void *addr, void *data)
{
	...
	return arch_ptrace_setreg(thread, (unsigned long)addr,
				  (unsigned long)data);
}
		...
		void *addr, void *data)
{
	if (req == CHCORE_PTRACE_ATTACH)
		return ptrace_init((int)cap);
	else
		return ptrace_ops(req, (int)cap, tid, addr, data);
}
